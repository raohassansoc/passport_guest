...
const GuestList = require("../../models/Booking/GuestList")
...
const BookingApi = () => {
    ...
    validationService
      ...
      .then(async (reqData) => {
        ...
        try {
          ...
          let final_amount = (epv_data.data[0].sell_price) * req.body.ticket_qty;
          if (
          ...
          let ticket_id = []
          for (let i = 0; i < saved_booking.length; i++) {
            ticket_id.push(saved_booking[i]._id)
            let webview_url = `https://passportv3.io/booking-details/${saved_booking[i]._id}`;
            let qr_url = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(
              webview_url
            )}&format=jpeg`;
            await Booking.updateOne({ _id: saved_booking[i]._id }, { $set: { qr_url: qr_url } });
          }

          await GuestList.findOneAndUpdate(
            { event_id: saved_booking[0].event_id, user: saved_booking[0].user_id },
            { $addToSet: { Booking: ticket_id } },
  ...
  const get = async (req, res) => {
    try {
      ...
      if (req.query.event_id) {
        let total_booking = response.data;


        function filterRedundantObjects(array) {

          let uniqueIds = {}; // Object to store unique user_ids
          let result = [];    // Array to store non-redundant objects

          // Iterate through each object in the array
          array.forEach(obj => {
            // If user_id is not encountered yet, add it to uniqueIds and push the object to result
            if (!uniqueIds[obj.user_id]) {

              uniqueIds[obj.user_id] = 1;

              let ticket_count = array.filter((item) => {
                return JSON.stringify(item.user_id) == JSON.stringify(obj.user_id);
              });

              obj.total_booking = ticket_count.length
              result.push(obj);
            }
        ...
        // Call the function to filter redundant objects
        let filteredArray = filterRedundantObjects(total_booking);

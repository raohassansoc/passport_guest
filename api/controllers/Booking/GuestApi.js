...
const GuestApi = () => {
      ...
      .then(async (reqData) => {
        try {
          ...
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Guest Details ${reqData._id ? "updated" : "created"
              } successfully.`,
            data: response || {},
          });
        } catch (error) {
  ...
  const getGuestList = async (req, res) => {
    ...
    try {
      const guestListByEvent = await GuestList.find({
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Guest list Details get successfully.",
        data: guestListByEvent,

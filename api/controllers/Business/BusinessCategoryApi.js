...
const BusinessCategory = require("../../models/Business/BusinessCategory");
const { BusinessCategorySchema } = require("../../schemas/BusinessSchema");
const Sentry = require("@sentry/node");

const BusinessCategoryApi = () => {
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, BusinessCategorySchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id)
          ...
          else response = await crudServices.insert(BusinessCategory, reqData);
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(BusinessCategory, {
...
module.exports = BusinessCategoryApi;

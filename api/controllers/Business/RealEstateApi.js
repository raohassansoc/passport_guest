...
const RealEstateApi = () => {
  const save = async (req, res) => {
    ...
    if (req.body._id) {
      ...
      let found_data = await crudServices.get(RealEstate, executing_parameters);
      req.body.images = [];
      if (
    ...
    validationService
      .validate(req.body, RealEstateSchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id)
          ...
          else response = await crudServices.insert(RealEstate, reqData);
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Real Estate Details ${
  ...
  const get = async (req, res) => {
    try {
      ...
      let response = await crudServices.get(RealEstate, executing_parameters);
      let page_info = {};
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Real Estate Details get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(RealEstate, {
          _id: req.body.record_id,
        });
        return res.status(200).json({
          code: 200,
          success: true,
          message: `Real Estate Details deleted successfully.`,
          data: {},
        });
      } else {

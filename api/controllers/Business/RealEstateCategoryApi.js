...
const { RealEstateCategorySchema } = require("../../schemas/BusinessSchema");

const RealEstateCategory = require("../../models/Business/RealEstateCategory");
const Sentry = require("@sentry/node");

const RealEstateCategoryApi = () => {
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, RealEstateCategorySchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id)
            ...
            response = await crudServices.insert(RealEstateCategory, reqData);
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Real Estate Category ${
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(RealEstateCategory, {
...
module.exports = RealEstateCategoryApi;

...
const { DesignationSchema } = require("../../schemas/EmployeeSchema");
...
const DesignationApi = () => {
  const save = async (req, res) => {
    ...
    validationService
      .validate(req.body, DesignationSchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id) {
            ...
            response = await crudServices.insert(Designation, reqData);
          }
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Employee Designation Details ${
  ...
  const get = async (req, res) => {
    try {
      ...
      let response = await crudServices.get(Designation, executing_parameters);
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Employee Designation get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(Designation, {

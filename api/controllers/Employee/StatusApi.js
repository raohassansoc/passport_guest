...
const { StatusSchema } = require("../../schemas/EmployeeSchema");
const Status = require("../../models/Employee/Status");
...
const StatusApi = () => {
  const save = async (req, res) => {
    ...
    validationService
      .validate(req.body, StatusSchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id) {
            ...
            response = await crudServices.insert(Status, reqData);
          }
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Employee Status Details ${
  ...
  const get = async (req, res) => {
    try {
      ...
      let response = await crudServices.get(Status, executing_parameters);
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Employee Status get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(Status, {

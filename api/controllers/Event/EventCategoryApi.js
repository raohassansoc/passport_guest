...
const { EventCategorySchema } = require("../../schemas/EventSchema");
...
const EventCategoryApi = () => {
  const save = async (req, res) => {
    ...
    validationService
      .validate(req.body, EventCategorySchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id)
          ...
          else response = await crudServices.insert(EventCategory, reqData);
  ...
  const get = async (req, res) => {
    try {
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Event Category get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(EventCategory, {
...
module.exports = EventCategoryApi;

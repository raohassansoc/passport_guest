const EventHostCategory = require("../../models/Event/EventHostCategory");
const { EventHostCategorySchema } = require("../../schemas/EventSchema");
const validationService = require("../../services/validation.service");
const crudServices = require("../../services/mongo.crud.services");
const { ObjectId } = require("mongodb");
const Sentry = require("@sentry/node");

const EventHostCategoryApi = () => {
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, EventHostCategorySchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id)
          ...
          else response = await crudServices.insert(EventHostCategory, reqData);
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Event Host Category ${
  ...
  const get = async (req, res) => {
    try {
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Event Host Category get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(EventHostCategory, {
          _id: req.body.record_id,
        });
        return res.status(200).json({
          code: 200,
          success: true,
          message: `Event Host Category deleted successfully.`,
          data: {},
        });
      } else {
...
module.exports = EventHostCategoryApi;

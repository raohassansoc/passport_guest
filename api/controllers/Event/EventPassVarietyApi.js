...
const { EventPassVarietySchema } = require("../../schemas/EventSchema");
const EventPassVariety = require("../../models/Event/EventPassVeriety");
const Sentry = require("@sentry/node");

const EventPassVarietyApi = () => {
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, EventPassVarietySchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id) {
            ...
            response = await crudServices.insert(EventPassVariety, reqData);
          }
          return res.status(201).json({
            code: 201,
            success: true,
            message: `Event Pass Variety Details ${
  ...
  const get = async (req, res) => {
    try {
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Event Pass Variety Details get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(EventPassVariety, {
          _id: req.body.record_id,
        });
        return res.status(200).json({
          code: 200,
          success: true,
          message: `Event Pass Variety Details deleted successfully.`,
          data: {},
        });
      } else {
...
module.exports = EventPassVarietyApi;

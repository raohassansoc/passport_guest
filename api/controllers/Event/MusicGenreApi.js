...
const MusicGenre = require("../../models/Event/MusicGenre");
const { MusicGenreSchema } = require("../../schemas/EventSchema");
...
const MusicGenreApi = () => {
  const save = async (req, res) => {
    ...
    validationService
      .validate(req.body, MusicGenreSchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id)
          ...
          else response = await crudServices.insert(MusicGenre, reqData);
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Event Music Genre ${
  ...
  const get = async (req, res) => {
    try {
      ...
      let response = await crudServices.get(MusicGenre, executing_parameters);
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Event Music Genre get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(MusicGenre, {
          _id: req.body.record_id,
        });
        return res.status(200).json({
          code: 200,
          success: true,
          message: `Event Music Genre deleted successfully.`,
          data: {},
        });
      } else {

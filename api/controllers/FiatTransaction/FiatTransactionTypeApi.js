...
const FiatTransactionType = require("../../models/FiatTransaction/FiatTransactionType");
const {
  FiatTransactionTypeSchema,
} = require("../../schemas/FiatTransactionSchema");
const Sentry = require("@sentry/node");

const FiatTransactionTypeApi = () => {
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, FiatTransactionTypeSchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id)
            ...
            response = await crudServices.insert(FiatTransactionType, reqData);
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Fiat Transaction Type ${
  ...
  const get = async (req, res) => {
    try {
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Fiat Transaction Type get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(FiatTransactionType, {
          _id: req.body.record_id,
        });
        return res.status(200).json({
          code: 200,
          success: true,
          message: `Fiat Transaction Type deleted successfully.`,
          data: {},
        });
      } else {
...
module.exports = FiatTransactionTypeApi;

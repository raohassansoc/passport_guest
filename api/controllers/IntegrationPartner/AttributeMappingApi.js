...
const AttributeMappingApi = () => {
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, AttributeMappingSchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id) {
            ...
            response = await crudServices.insert(AttributeMapping, reqData);
          }
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(AttributeMapping, {
          _id: req.body.record_id,
        });
        return res.status(200).json({
          code: 200,
          success: true,
          message: `Attribute Mapping Details deleted successfully.`,
          data: {},
        });
      } else {
  ...
  const get = async (req, res) => {
    try {
      ...
      let response = await crudServices.get(AttributeMapping, {
        where: whereClause,
        projection: {
          __v: 0,
          is_deleted: 0,
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: `Attribute Mapping Details get successfully.`,
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
...
module.exports = AttributeMappingApi;

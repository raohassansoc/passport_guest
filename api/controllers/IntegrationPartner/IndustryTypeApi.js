...
const IndustryType = require("../../models/IntegrationPartner/IndustryType");
const {
  IndustryTypeSchema,
...
const IndustryTypeApi = () => {
  const save = async (req, res) => {
    ...
    validationService
      .validate(req.body, IndustryTypeSchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id) {
            ...
            response = await crudServices.insert(IndustryType, reqData);
          }
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(IndustryType, { _id: req.body.record_id });
  ...
  const get = async (req, res) => {
    try {
      ...
      let response = await crudServices.get(IndustryType, {
        where: whereClause,
        projection: {
          is_deleted: 0,
          deleted_at: 0,
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: `Industry Type get successfully.`,
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
...
module.exports = IndustryTypeApi;

...
const IntegrationPartnerStatus = require("../../models/IntegrationPartner/IntegrationPartnerStatus");
const {
  IntegrationPartnerStatusSchema,
} = require("../../schemas/IntegrationPartnerSchema");
const Sentry = require("@sentry/node");

const IntegrationPartnerStatusApi = () => {
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, IntegrationPartnerStatusSchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id) {
          ...
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Integration Partner Status ${
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(IntegrationPartnerStatus, {
          _id: req.body.record_id,
        });
        return res.status(200).json({
          code: 200,
          success: true,
          message: `Integration Partner Status deleted successfully.`,
          data: {},
        });
      } else {
  ...
  const get = async (req, res) => {
    try {
      ...
      let response = await crudServices.get(IntegrationPartnerStatus, {
        where: whereClause,
        projection: {
          is_deleted: 0,
          deleted_at: 0,
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: `Integration Partner Status get successfully.`,
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
...
module.exports = IntegrationPartnerStatusApi;

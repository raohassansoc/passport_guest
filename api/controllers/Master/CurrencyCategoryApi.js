...
const CurrencyCategoryModel = require("../../models/Master/CurrencyCategory");
const { CurrencyCategorySchema } = require("../../schemas/MasterSchemas");
const { ObjectId } = require("mongodb");
const Sentry = require("@sentry/node");

const CurrencyCategoryApi = () => {
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, CurrencyCategorySchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id)
            response = await crudServices.update(
              CurrencyCategoryModel,
              { _id: reqData._id },
              reqData
            );
          else
  ...
  const get = async (req, res) => {
    try {
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Currency Category get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(CurrencyCategoryModel, {
          _id: req.body.record_id,
        });
        return res.status(200).json({
          code: 200,
          success: true,
          message: `Currency Category deleted successfully.`,
          data: {},
        });
      } else {
...
module.exports = CurrencyCategoryApi;

const EquityModel = require("../../models/Master/Equity");
const { EquitySchemas } = require("../../schemas/MasterSchemas");
...
const EquityApi = () => {
  // For create and update Equity
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, EquitySchemas)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id) {
            ...
            response = await crudServices.insert(EquityModel, reqData);
          }
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(EquityModel, { _id: req.body.record_id });
  ...
  const get = async (req, res) => {
    try {
      ...
      let response = await crudServices.get(EquityModel, {
        where: whereClause,
        projection: {
          _id: 1,
          total_equity: 1,

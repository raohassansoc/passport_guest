...
const AccountType = require("../../models/Merchant/AccountType");
const { AccountTypeSchema } = require("../../schemas/MerchantSchemas");
...
const AccountTypeApi = () => {
  const save = async (req, res) => {
    ...
    validationService
      .validate(req.body, AccountTypeSchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id) {
            ...
            response = await crudServices.insert(AccountType, reqData);
          }
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(AccountType, { _id: req.body.record_id });
  ...
  const get = async (req, res) => {
    try {
      ...
      let response = await crudServices.get(AccountType, {
        where: whereClause,
        projection: {
          is_deleted: 0,
          deleted_at: 0,

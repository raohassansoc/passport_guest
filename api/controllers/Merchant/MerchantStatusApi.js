const MerchantStatus = require("../../models/Merchant/MerchantStatus");
const { MerchantStatusSchemas } = require("../../schemas/MerchantSchemas");
...
const MerchantStatusApi = () => {
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, MerchantStatusSchemas)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id)
          ...
          else response = await crudServices.insert(MerchantStatus, reqData);
  ...
  const get = async (req, res) => {
    try {
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Merchant Status get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(MerchantStatus, {
          _id: req.body.record_id,
        });
        return res.status(200).json({
          code: 200,
          success: true,
          message: `Merchant Status deleted successfully.`,
          data: {},
        });
      } else {
...
module.exports = MerchantStatusApi;

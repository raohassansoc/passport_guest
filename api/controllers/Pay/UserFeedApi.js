...
const UserFeed = require("../../models/Pay/UserFeed");
const { ObjectId } = require("mongodb");
const { UserFeedSchema } = require("../../schemas/PaySchema");
...
const UserFeedApi = () => {
    const save = async (req, res) => {
        ...
        validationService
            .validate(req.body, UserFeedSchema)
            .then(async (reqData) => {
                try {
                    let response;
                    if (reqData._id)
                    ...
                    else response = await crudServices.insert(UserFeed, reqData);
                    return res.status(201).json({
                        code: 200,
                        success: true,
                        message: `User Feed ${reqData._id ? "updated" : "created"
                            } successfully.`,
                        data: response || {},
                    });
                } catch (error) {
    ...
    const get = async (req, res) => {
        try {
            ...
            const executing_parameters = {
                ...
                skip: skip,
                limit: limit,
                projection: { _id: 1, image: 1, user_id: 1 },
            };
            ...
            return res.status(200).json({
                code: 200,
                success: true,
                message: "User Feed get successfully.",
                data: response.data,
                page_info: page_info,
            });
        } catch (error) {
    ...
    const destroy = async (req, res) => {
        try {
            if (req.body.record_id) {
                await crudServices.destroy(UserFeed, {
                    _id: req.body.record_id,
                });
                return res.status(200).json({
                    code: 200,
                    success: true,
                    message: `User Feed deleted successfully.`,
                    data: {},
                });
            } else {

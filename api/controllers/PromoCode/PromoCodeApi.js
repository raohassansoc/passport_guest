      ...
      .then(async (reqData) => {
        try {
          ...
          if (reqData._id) {
            ...
            let arr = []
            const event_list_ids = ObjectId(JSON.parse(reqData.event_list_ids))
            for (let i = 0; i <= reqData.promo_code_qty; i++) {
              const data = { ...reqData };
              const token = generateRandomHex(8)
              console.log(token)
              data.code = token
              data.event_list_ids = event_list_ids
              delete data._id
              arr.push(data)
            }
            console.log("arr", arr)
            response = await PromoCode.insertMany(arr);
          }
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Promo Code Details ${reqData._id ? "updated" : "created"
              } successfully.`,
            data: response || {},
          });
        } catch (error) {

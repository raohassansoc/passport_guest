...
const { CuisineSchema } = require("../../schemas/RestaurantSchema");
...
const CuisineApi = () => {
  const save = async (req, res) => {
    ...
    validationService
      .validate(req.body, CuisineSchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id) {
            ...
            response = await crudServices.insert(Cuisine, reqData);
          }
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Cuisine Details ${
  ...
  const get = async (req, res) => {
    try {
      ...
      let response = await crudServices.get(Cuisine, executing_parameters);
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Cuisine Details get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(Cuisine, {
          _id: req.body.record_id,
        });
        return res.status(200).json({
          code: 200,
          success: true,
          message: `Cuisine Details deleted successfully.`,
          data: {},
        });
      } else {

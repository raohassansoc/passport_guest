...
const RatingCategory = require("../../models/Restaurant/RatingCategory");
const { RatingCategorySchema } = require("../../schemas/RestaurantSchema");
const Sentry = require("@sentry/node");

const RatingCategoryApi = () => {
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, RatingCategorySchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id) {
            ...
            response = await crudServices.insert(RatingCategory, reqData);
          }
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Rating Category Details ${
  ...
  const get = async (req, res) => {
    ...
    try {
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Rating Category get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(RatingCategory, {
...
module.exports = RatingCategoryApi;

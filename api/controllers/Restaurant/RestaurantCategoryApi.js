...
const { RestaurantCategorySchema } = require("../../schemas/RestaurantSchema");
const Sentry = require("@sentry/node");

const RestaurantCategoryApi = () => {
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, RestaurantCategorySchema)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id) {
            ...
            response = await crudServices.insert(RestaurantCategory, reqData);
          }
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Restaurant Category Details ${
  ...
  const get = async (req, res) => {
    ...
    try {
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: "Restaurant Category get successfully.",
        data: response.data,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(RestaurantCategory, {
...
module.exports = RestaurantCategoryApi;

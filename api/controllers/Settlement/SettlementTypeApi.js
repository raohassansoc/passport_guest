const SettlementType = require("../../models/Settlement/SettlementType");
const { SettlementTypeSchemas } = require("../../schemas/SettlementSchemas");
...
const SettlementTypeApi = () => {
  const save = async (req, res) => {
    await validationService.convertIntObj(req.body);
    validationService
      .validate(req.body, SettlementTypeSchemas)
      .then(async (reqData) => {
        try {
          let response;
          if (reqData._id) {
            ...
            response = await crudServices.insert(SettlementType, reqData);
          }
          return res.status(201).json({
            code: 200,
            success: true,
            message: `Settlement Type ${
  ...
  const get = async (req, res) => {
    try {
      ...
      return res.status(200).json({
        code: 200,
        success: true,
        message: `Settlement Type get successfully.`,
        data: response,
        page_info: page_info,
      });
    } catch (error) {
  ...
  const destroy = async (req, res) => {
    try {
      if (req.body.record_id) {
        await crudServices.destroy(SettlementType, { _id: req.body.record_id });
        return res.status(200).json({
          code: 200,
          success: true,
          message: `Settlement Type deleted successfully.`,
          data: {},
        });
      } else {
...
module.exports = SettlementTypeApi;
